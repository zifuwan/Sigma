...
class Evaluator(object):
    def __init__(self, dataset, class_num, norm_mean, norm_std, network, multi_scales, 
        ...
        self.batch_size = 32  # Define the batch size

        self.context = mp.get_context('spawn')
        self.val_func = network  # Assuming network is already loaded with weights if necessary
    ...
    def single_process_evalutation(self):
        ...
        for idx in tqdm(range(self.ndata)):
            dd = self.dataset[idx]
            results_dict = self.func_per_iteration(dd)
    ...
    def val_func_process(self, batch_input_data, device=None):
        input_data = torch.FloatTensor(batch_input_data).cuda(device)
        with torch.no_grad():
            self.val_func.eval()
            self.val_func.to(device)
            score_batch = self.val_func(input_data)
            if self.is_flip:
                input_data_flipped = torch.flip(input_data, [-1])
                score_flipped = self.val_func(input_data_flipped)
                score_batch += torch.flip(score_flipped, [-1])
        return score_batch  # Assuming this returns a list of results corresponding to the batch
